...
REDSTATUS RedVolMountMaster(void)
{
    ...
    /*  Read the master block, to ensure that the disk was formatted with
        Reliance Edge.
    */
    ret = RedBufferGet(0U, BFLAG_META_MASTER, CAST_VOID_PTR_PTR(&pMB));
...
REDSTATUS RedVolMountMetaroot(void)
{
    ...
    ret = RedIoRead(gbRedVolNum, 1U, 1U, &gpRedCoreVol->aMR[0U]);

    if(ret == 0)
    {
        ret = RedIoRead(gbRedVolNum, 2U, 1U, &gpRedCoreVol->aMR[1U]);
    }
...
REDSTATUS RedVolTransact(void)
{
    ...
    if(gpRedCoreVol->fBranched)
    {
        ...
        if(ret == 0)
        {
            ret = RedIoWrite(gbRedVolNum, 1UL + gpRedCoreVol->bCurMR, 1U, gpRedMR);
