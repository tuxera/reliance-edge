...
#define DIRENTS_MAX         (uint32_t)REDMIN(UINT32_MAX, (INODE_SIZE_MAX / DIRENT_SIZE))
    ...
    uint32_t    ulInode)
{
    ...
    if((pPInode == NULL) || (pPInode->pBuffer == NULL) || !pPInode->fDirty || (pszName == NULL))
    {
        ret = -RED_EINVAL;
    }
    else
...
/** @brief Delete an existing directory entry.
    ...
    @retval 0           Operation was successful.
    @retval -RED_EIO    A disk I/O error occurred.
    @retval -RED_ENOSPC The file system does not have enough space to modify the
    ...
    uint32_t    ulDeleteIdx)
{
    ...
    if((pPInode == NULL) || (pPInode->pBuffer == NULL) || !pPInode->fDirty || (ulDeleteIdx >= DIRENTS_MAX))
    {
        ret = -RED_EINVAL;
    }
    else if((DirEntryIndexToOffset(ulDeleteIdx) + DIRENT_SIZE) == pPInode->pBuffer->ullSize)
    {
    ...
    uint32_t   *pulInode)
{
    ...
    if((pPInode == NULL) || (pPInode->pBuffer == NULL) || (pszName == NULL))
    {
        ret = -RED_EINVAL;
    }
    else
    {
        ...
        {
            uint32_t    ulIdx = 0U;
            uint32_t    ulDirentCount = DirOffsetToEntryIndex(pPInode->pBuffer->ullSize);
                ...
                if(ret == 0)
                {
                    ...
                    for(ulBlockIdx = 0U; ulBlockIdx < ulBlockLastIdx; ulBlockIdx++)
                    {
                        ...
                        if(    (pDirent->ulInode != INODE_INVALID)
                            && (RedStrNCmp(pDirent->szName, pszName, ulNameLen) == 0)
                        ...
                        if((ulFreeIdx == DIR_INDEX_INVALID) && (pDirent->ulInode == INODE_INVALID))
                        {
...
/** @brief Read the next entry from a directory, given a starting index.
    ...
    @retval -RED_EINVAL @p pPInode is not a mounted cached inode structure; or
                        @p pszName is `NULL`; or @p pulIdx is `NULL`; or
                        @p pulInode is `NULL`.
*/
    ...
    uint32_t   *pulInode)
{
    ...
    if((pPInode == NULL) || (pPInode->pBuffer == NULL) || (pulIdx == NULL) || (pszName == NULL) || (pulInode == NULL))
    {
        ret = -RED_EINVAL;
    }
    else
        ...
        while((ret == 0) && (ulIdx < ulDirentCount))
        {
            ...
            if(ret == 0)
            {
                ...
                for(ulBlockIdx = ulIdx % DIRENTS_PER_BLOCK; ulBlockIdx < ulBlockLastIdx; ulBlockIdx++)
                {
                    ...
                    {
                        *pulIdx = ulIdx + 1U;
                        RedStrNCpy(pszName, pDirents[ulBlockIdx].szName, REDCONF_NAME_MAX);
    ...
    CINODE     *pDstInode)
{
    ...
    if(    (pSrcPInode == NULL)
        || (pSrcPInode->pBuffer == NULL)
        || !pSrcPInode->fDirty
        || (pSrcInode == NULL)
        || (pszSrcName == NULL)
        || (pDstPInode == NULL)
        || (pDstPInode->pBuffer == NULL)
        || !pDstPInode->fDirty
        || (pszDstName == NULL)
        || (pDstInode == NULL))
    {
        ...
        */
      ...
      #endif
        {
            ...
            if((ret == 0) && (pDstInode->ulInode != INODE_INVALID))
            {
                ...
                */
                if((ret == 0) && pDstInode->fDirectory && (pDstInode->pBuffer->ullSize > 0U))
                {
                    ret = -RED_ENOTEMPTY;
                }
            }
            ...
            */
            ...
            if(ret == 0)
            {
                pSrcInode->pBuffer->ulPInode = pDstPInode->ulInode;
            }
        }
    }
    ...
    const CINODE   *pDstPInode)
{
    ...
    if(ulSrcInode == pDstPInode->ulInode)
    {
        ret = -RED_EINVAL;
    }
    else
    {
        ...
        NextParent.ulInode = pDstPInode->pBuffer->ulPInode;

        while((NextParent.ulInode != ulSrcInode) && (NextParent.ulInode != INODE_ROOTDIR) && (NextParent.ulInode != INODE_INVALID))
        {
            ...
            NextParent.ulInode = NextParent.pBuffer->ulPInode;
    ...
    uint32_t    ulNameLen)
{
    ...
    RedStrNCpy(de.szName, pszName, ulNameLen);

    return RedInodeDataWrite(pPInode, ullOffset, &ulLen, &de);
}
