...
#define MAXNAMELEN REDCONF_NAME_MAX
...
static void show_ops(int flag, const char *lead_str);
...
static void write_freq(void);
static void zero_freq(void);
...
int fsstress_main(int argc, char **argv)
{
    ...
    myprog = argv[0];
    for (i = 1; i < argc; i++) {
        const char *const opts = "cd:f:i:l:n:rs:vwzHS";
        char *thisarg = argv[i];
        char *optarg = NULL;
        char *found;

        /*  Rudimentary simulation of getopt().
        */
        c = '?';
        if (*thisarg == '-') {
            c = thisarg[1];
...
static void process_freq(char *arg)
{
    ...
    *s++ = '\0';
    ...
    RedPrintf("can't find op type %s for -f\n", arg);
    exit(1);
}
...
static void usage(void)
{
    RedPrintf("Usage: %s -H   or\n", myprog);
    RedPrintf
        ("       %s [-c][-d dir][-f op_name=freq][-l loops][-n nops]\n",
         myprog);
    RedPrintf("          [-r len][-s seed][-v][-w][-z][-S]\n");
    RedPrintf("where\n");
    RedPrintf
