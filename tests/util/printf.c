...
static void Uint64DivMod(uint64_t ullDividend, uint32_t ulDivisor, uint64_t *pullQuotient, uint32_t *pulRemainder);
static int32_t AtoL(const char *pach);
        ...
        */
        ...
        else
        {
            ...
            */
            ...
            {
                pFormatType->ulFillLen = (uint32_t)AtoL(&pszFomat[ulIdx]);
    ...
    else
    {
        ...
        */
        do
        {
            ...
            /*  Note: Uint64DivMod() is smart enough to use normal division
                once ullNumericVal <= UINT32_MAX.
            */
            Uint64DivMod(ullNum, 10U, &ullQuotient, &ulRemainder);
            ...
            */
            do
            {
                ...
                /*  Note: Uint64DivMod() is smart enough to use normal division
                    once ullNumericVal <= UINT32_MAX.
                */
                Uint64DivMod(ullNumericVal, 10U, &ullQuotient, &ulRemainder);
...
/** @brief Divide a 64-bit value by a 32-bit value, returning the quotient and
    ...
    @param pullQuotient Populated with the result of the division.
    @param pulRemander  Populated with the remainder.
*/
static void Uint64DivMod(
    uint64_t    ullDividend,
    uint32_t    ulDivisor,
    uint64_t   *pullQuotient,
    uint32_t   *pulRemainder)
{
    ...
    */
    if(ulDivisor == 0)
    {
        ...
        */
        *pullQuotient = 0xFFFFFFFFFFFFFBADULL;
        *pulRemainder = 0xFFFFFBADU;
    }
    else if(ullDividend <= UINT32_MAX)
    {
        uint32_t ulDividend = (uint32_t)ullDividend;

        *pullQuotient = ulDividend / ulDivisor;
        *pulRemainder = ulDividend % ulDivisor;
    }
    else
    {
        uint32_t    ulResultHi;
        ...
        *pullQuotient = ((uint64_t)ulResultHi << 32U) + ulResultLo;
        *pulRemainder = (uint32_t)(ullDividend - (*pullQuotient * ulDivisor));
    }
}
...
/** @brief Converts an ASCII number into an int32_t.
    ...
    @param pach Pointer to a constant array of characters.
