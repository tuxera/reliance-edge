...
#include <redvolume.h>
#include <redposix.h>
#include "redposixutil.h"


static bool IsRootDir(const char *pszPath);
    ...
    const char    **ppszLocalPath)
{
    ...
    else
    {
        ...
        uint32_t    ulPathLen = RedStrLen(pszPath);
        ...
        for(bVolNum = 0U; bVolNum < REDCONF_VOLUME_COUNT; bVolNum++)
        {
            ...
            /*  For a path to match, it must either be the prefix exactly, or
                ...
                "/foobar" is not.
            */
            else if(    (ulPrefixLen <= ulPathLen)
                     && ((pszPath[ulPrefixLen] == '\0') || (pszPath[ulPrefixLen] == REDCONF_PATH_SEPARATOR))
                     && (RedStrNCmp(pszPath, pszPrefix, ulPrefixLen) == 0))
            {
        ...
        if(ret == 0)
        {
            ...
            else
            {
                ...
                */
                if((pszLocalPath[0U] != '\0') && !IsRootDir(pszLocalPath))
                {
                    ret = -RED_ENOENT;
                }
            }
...
/** @brief Lookup the inode named by the given path.
    ...
    @param pszPath  The path to lookup; this is a local path, without any volume
                    prefix.
    @param pulInode On successful return, populated with the number of the inode
                    named by pszPath.
    ...
    const char    **ppszName)
{
    ...
    if((pszLocalPath == NULL) || (pulPInode == NULL) || (ppszName == NULL) || IsRootDir(pszLocalPath))
    {
        ret = -RED_EINVAL;
    }
    else if(pszLocalPath[0U] == '\0')
    ...
    else
    {
        ...
        do
        {
            ...
            */
            if(pszLocalPath[ulPathIdx + ulNameLen] == REDCONF_PATH_SEPARATOR)
            {
    ...
    const char *pszPath)
{
    ...
    /*  A string containing nothing but path separators (usually only one)
        ...
        errors.
    */
    while(pszPath[ulIdx] == REDCONF_PATH_SEPARATOR)
    {
        ulIdx++;
    }

    return (ulIdx > 0U) && (pszPath[ulIdx] == '\0');
}
