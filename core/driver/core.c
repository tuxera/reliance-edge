...
const VOLCONF  *gpRedVolConf = &gaRedVolConf[0U];
VOLUME         *gpRedVolume = &gaRedVolume[0U];
COREVOLUME     *gpRedCoreVol = &gaCoreVol[0U];
...
REDSTATUS RedCoreInit(void)
{
    ...
    */
    ...
    for(bVolNum = 0U; bVolNum < REDCONF_VOLUME_COUNT; bVolNum++)
    {
        ...
        if((pVolConf->ulSectorSize == 0U) || (pVolConf->ulSectorSize > REDCONF_BLOCK_SIZE) || (pVolConf->ulInodeCount == 0U))
        {
            ret = -RED_EINVAL;
        }
        ...
        {
            /*  Ensure there are no duplicate path prefixes.
            */
          #if REDCONF_VOLUME_COUNT > 1U
            uint8_t bCmpVol;

            /*  Check against all previous volumes, which are already verified.
            */
        ...
        if(ret == 0)
        {
            ...
            else
            {
                ...
                */
                ...
                /*  The imap needs bits for all inode and allocable blocks.  If
                    ...
                    used and there are no imap nodes on disk.
                */
                ...
                else
                {
                    ...
                    /*  The imap does not include bits for itself, so the number
                        of imap nodes required must be computed iteratively.
                    */
                    pCoreVol->ulImapNodeCount = 1U;
                    while(((pVol->ulBlockCount - 3U) - (pCoreVol->ulImapNodeCount * 2U)) > (IMAPNODE_ENTRIES * pCoreVol->ulImapNodeCount))
                    {
            ...
            if(ret == 0)
            {
                ...
                {
                    /*  We can get here if there is not enough space for the
                        number of configured inodes.
                    */
                    ret = -RED_EINVAL;
                }
                else
...
#if (REDCONF_READ_ONLY == 0) && ((REDCONF_API_FSE == 0) || (REDCONF_API_FSE_TRANSMASKSET == 1))
...
#if (REDCONF_API_FSE == 0) || (REDCONF_API_FSE_TRANSMASKGET == 1)
    ...
    uint32_t   *pulInode)
{
    ...
    if((pulInode == NULL) || (pszName == NULL) || (pszName[0U] == '\0') || !gpRedVolume->fMounted)
    {
        ret = -RED_EINVAL;
    }
    else if(gpRedVolume->fReadOnly)
    ...
    uint32_t    ulInode)
{
    ...
    if((pszName == NULL) || (pszName[0U] == '\0') || !gpRedVolume->fMounted)
    {
        ret = -RED_EINVAL;
    }
    else if(gpRedVolume->fReadOnly)
    ...
    else
    {
        ...
        if(ret == 0)
        {
            ...
            */
            ...
            if(ret == 0)
            {
                if(ino.pBuffer->uNLink == UINT16_MAX)
                {
                    ret = -RED_EMLINK;
                }
                else
    ...
    const char *pszName)
{
    ...
    if((pszName == NULL) && !gpRedVolume->fMounted)
    {
        ret = -RED_EINVAL;
    }
    else if(gpRedVolume->fReadOnly)
    ...
    const char *pszName)
{
    ...
    else
    {
        ...
        pino.ulInode = ulPInode;
        ret = RedInodeMount(&pino, FTYPE_EITHER, false);
        ...
        if(ret == 0)
        {
            ...
            if(ret == 0)
            {
                ...
                if(ret == 0)
                {
                    if(ino.fDirectory && (ino.pBuffer->ullSize > 0U))
                    {
                        ret = -RED_ENOTEMPTY;
                    }
                    else
    ...
    REDSTAT    *pStat)
{
    ...
    else
    {
        ...
        if(ret == 0)
        {
            ...
            pStat->st_mode = ino.pBuffer->uMode;
          #if REDCONF_API_POSIX_LINK == 1
            pStat->st_nlink = ino.pBuffer->uNLink;
          #else
            pStat->st_nlink = 1U;
          #endif
            pStat->st_size = ino.pBuffer->ullSize;
          #if REDCONF_INODE_TIMESTAMPS == 1
            pStat->st_atime = ino.pBuffer->ulATime;
            pStat->st_mtime = ino.pBuffer->ulMTime;
            pStat->st_ctime = ino.pBuffer->ulCTime;
          #endif
          #if REDCONF_INODE_BLOCKS == 1
            pStat->st_blocks = ino.pBuffer->ulBlocks;
          #endif
    ...
    uint64_t    ullSize)
{
    ...
    else
    {
        ...
        {
          #if RESERVED_BLOCKS > 0U
            gpRedCoreVol->fUseReservedBlocks = (ullSize < ino.pBuffer->ullSize);
          #endif
