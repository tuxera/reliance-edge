...
/*  A note on the typecasts in the below macro: Operands to bitwise operators
...
static void BufferFinalize(uint8_t *pbBuffer, uint16_t uFlags);
#endif
    ...
    void      **ppBuffer)
{
    ...
    else
    {
        ...
        /*  Search for the least recently used buffer which is not referenced.
        */
        ...
        if(ret == 0)
        {
            if((uFlags & BFLAG_NEW) == 0U)
            {
                /*  If the read fails, the old contents of the LRU may no
                    longer be there.
                */
                ...
                if((ret == 0) && ((uFlags & BFLAG_META) != 0U) && !BufferIsValid(gBufCtx.b.aabBuffer[bIdx], uFlags))
                {
                    ret = -RED_EIO;
                }
    ...
    uint8_t    *pbIdx)
{
    ...
    if((pBuffer != NULL) && (pbIdx != NULL))
    {
        ...
        /*  pBuffer should be a pointer to one of the handles.
        ...
        */
        ...
        if((bIdx < REDCONF_BUFFER_COUNT) && (gBufCtx.aHead[bIdx].bVolNum == gbRedVolNum))
        {
            *pbIdx = bIdx;
            fRet = true;
        }
    ...
    uint8_t bIdx)
{
    ...
    if(bIdx < REDCONF_BUFFER_COUNT)
    {
        ...
        {
            BufferFinalize(gBufCtx.b.aabBuffer[bIdx], pHead->uFlags);
        }
    ...
    uint16_t uFlags)
{
    ...
    if((ulSignature == 0U) || (pbBuffer == NULL))
    {
        REDERROR();
    }
    else
    {
        ...
        RedMemCpy(&pbBuffer[NODEHEADER_OFFSET_SEQ], &gpRedVolume->ullSequence, sizeof(gpRedVolume->ullSequence));
    ...
    uint8_t bIdx)
{
    ...
    if(bIdx != gBufCtx.abMRU[REDCONF_BUFFER_COUNT - 1U])
    {
        /*  Find the current position of the buffer in the MRU array.
        */
    ...
    uint8_t *pbIdx)
{
    ...
    REDASSERT(ulBlock <= gpRedVolume->ulBlockCount);
