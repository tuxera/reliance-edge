...
REDSTATUS RedVolFormat(void)
{
    ...
    if(ret == 0)
    {
        ...
        */
        ret = RedBufferGet(0U, BFLAG_NEW | BFLAG_DIRTY, CAST_VOID_PTR_PTR(&pMB));

        if(ret == 0)
        {
            ret = RedBufferFlush(0U, 1U);
        ...
        if((ret == 0) && !gpRedCoreVol->fImapInline)
        {
            ...
            for(ulImapNode = 0U; ulImapNode < gpRedCoreVol->ulImapNodeCount; ulImapNode++)
            {
                uint8_t bWhichImap;

                /*  Technically it is only necessary to create one copy of each
                    imap node (the copy the metaroot points at), but creating
                    them both avoids headaches during disk image analysis from
                    stale imaps left over from previous formats.
                */
                for(bWhichImap = 0U; bWhichImap < 2U; bWhichImap++)
                {
                    uint32_t    ulBlock = gpRedCoreVol->ulImapStartBN + (ulImapNode * 2U) + bWhichImap;
                    IMAPNODE   *pImap;

                    ret = RedBufferGet(ulBlock, (uint16_t)((uint32_t)BFLAG_META_IMAP | BFLAG_NEW | BFLAG_DIRTY), CAST_VOID_PTR_PTR(&pImap));
                    if(ret != 0)
                    {
                        break;
                    }
        ...
        */
        if(ret == 0)
        {
            ret = RedBufferGet(0U, (uint16_t)((uint32_t)BFLAG_META_MASTER | BFLAG_NEW | BFLAG_DIRTY), CAST_VOID_PTR_PTR(&pMB));
        }
        ...
        if(ret == 0)
        {
            ...
            pMB->bBlockSizeP2 = 1U;
            while((1UL << pMB->bBlockSizeP2) != REDCONF_BLOCK_SIZE)
            {
                pMB->bBlockSizeP2++;
            }
...
#endif /* (REDCONF_READ_ONLY == 0) && (((REDCONF_API_POSIX == 1) && (REDCONF_API_POSIX_FORMAT == 1)) || (REDCONF_IMAGE_BUILDER == 1)) */
