...
typedef struct
{
    ...
    REDSTATUS   errno;      /**< Last error value. */
} TASKSLOT;
#endif
    ...
    if(ret == 0)
    {
        ...
        if(ret == -RED_EISDIR)
        {
            /*  POSIX says that if a file descriptor is not writable, the
                ...
                list -RED_EISDIR as an allowed errno.  Therefore -RED_EBAD
                takes precedence.
            */
            ret = -RED_EBADF;
        }
        ...
        */
        ...
        if(ret == 0)
        {
            switch(whence)
            {
                ...
                */
                case RED_SEEK_CUR:
                    REDASSERT(pHandle->ullOffset <= INT64_MAX);
        ...
        if(ret == 0)
        {
            ...
            if((llNewOffset < 0) || ((uint64_t)llNewOffset > gaRedVolume[pHandle->bVolNum].ullMaxInodeSize))
            {
                /*  Invalid file offset.
                */
                ret = -RED_EINVAL;
        ...
        if(ulIdx == REDCONF_TASK_COUNT)
        {
            ...
            */
            ...
            if(ret == 0)
            {
                ...
                REDASSERT(gaTask[ulIdx].errno == 0);

                piErrno = &gaTask[ulIdx].errno;
            }
            else
            {
                ...
                */
                if(!fCreated)
                {
                    ...
                    */
                    ...
                    {
                        ret = RedCoreFileTruncate(ulInode, 0ULL);
                    }
                  #endif
                }
                ...
                if(ret == 0)
                {
                    ...
                    *piFildes = FildesPack(uHandleIdx, bVolNum);
                }
            }
        }
    }
    ...
    uint8_t     bVolNum)
{
    ...
    REDASSERT(uHandleIdx <= FD_IDX_MAX);
