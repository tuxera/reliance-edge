...
/** @brief Pick which example implementation is compiled.
    ...
    - #BDEV_RAM_DISK
*/
#define BDEV_EXAMPLE_IMPLEMENTATION BDEV_ATMEL_SDMMC
    ...
    else
    {
        ...
        for(ulSectorIdx = 0U; ulSectorIdx < ulSectorCount; ulSectorIdx++)
        {
            iErr = pDriver->readsector(pDriver, &pbBuffer[ulSectorIdx * ulSectorSize],
                                       (unsigned long)(ullSectorStart + ulSectorCount));
            if(iErr != 0)
            {
                ret = -RED_EIO;
                break;
    ...
    const void *pBuffer)
{
    ...
    else
    {
        ...
        for(ulSectorIdx = 0U; ulSectorIdx < ulSectorCount; ulSectorIdx++)
        {
            ...
            */
            iErr = pDriver->writesector(pDriver, (uint8_t *)&pbBuffer[ulSectorIdx * ulSectorSize],
                                        (unsigned long)(ullSectorStart + ulSectorCount));
            if(iErr != 0)
            {
                ret = -RED_EIO;
    ...
    BDEVOPENMODE    mode)
{
    ...
    /*  Assuming that the volume number is also the correct drive number.  If
        this is not the case in your environment, a static constant array can be
        declared to map volume numbers to the correct driver number.
    */
      ...
      #endif
        {
            ...
            */
            if(    (gaRedVolConf[bVolNum].ulSectorSize != 512U)
                || ((ulSectorLast + 1U) < gaRedVolConf[bVolNum].ullSectorCount))
            {
                ret = -RED_EINVAL;
            }
        }
    ...
    BDEVOPENMODE    mode)
{
    ...
    {
        gapbRamDisk[bVolNum] = calloc(gaRedVolume[bVolNum].ulBlockCount, REDCONF_BLOCK_SIZE);
        if(gapbRamDisk[bVolNum] == NULL)
        {
            ret = -RED_EIO;
        }
    ...
    void       *pBuffer)
{
    ...
    else
    {
        ...
        memcpy(pBuffer, &gapbRamDisk[bVolNum][ullByteOffset], ulByteCount);
    ...
    const void *pBuffer)
{
    ...
    else
    {
        ...
        memcpy(&gapbRamDisk[bVolNum][ullByteOffset], pBuffer, ulByteCount);
