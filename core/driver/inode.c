...
static REDSTATUS InodeIsWriteable(uint32_t ulInode, bool *pfIsWriteable);
#endif
...
/** @brief Mount an existing inode.
    ...
    @retval -RED_EINVAL     @p pInode is `NULL`.
    ...
    bool        fBranch)
{
    ...
    else
    {
        ...
        if(ret == 0)
        {
            ret = RedBufferGet(InodeBlock(pInode->ulInode, bWhich), BFLAG_META_INODE, CAST_VOID_PTR_PTR(&pInode->pBuffer));
        }

      #if REDCONF_READ_ONLY == 0
        if(ret == 0)
        {
            ret = InodeIsWriteable(pInode->ulInode, &pInode->fWriteable);
        }
      #endif

        if(ret == 0)
        {
            if(RED_S_ISDIR(pInode->pBuffer->uMode))
            {
    ...
    uint16_t    uMode)
{
    ...
    else
    {
        ...
        if(ret == 0)
        {
            ...
            if(ret == 0)
            {
                ret = RedBufferGet(InodeBlock(pInode->ulInode, bWriteableWhich),
                    (uint16_t)((uint32_t)BFLAG_META_INODE | BFLAG_DIRTY | BFLAG_NEW), CAST_VOID_PTR_PTR(&pInode->pBuffer));
        ...
        if(ret == 0)
        {
            ...
            pInode->pBuffer->ulATime = ulNow;
            pInode->pBuffer->ulMTime = ulNow;
            pInode->pBuffer->ulCTime = ulNow;
          #endif

            pInode->pBuffer->uMode = uMode;

          #if REDCONF_API_POSIX == 1
          #if REDCONF_API_POSIX_LINK == 1
            pInode->pBuffer->uNLink = 1U;
          #endif
            pInode->pBuffer->ulPInode = ulPInode;
          #else
            (void)ulPInode;
          #endif

            pInode->fWriteable = true;
            pInode->fDirty = true;
...
#endif /* (REDCONF_READ_ONLY == 0) && ((REDCONF_API_POSIX == 1) || (REDCONF_IMAGE_BUILDER == 1)) */
    ...
    CINODE *pInode)
{
    ...
    {
        if(pInode->pBuffer->ullSize != 0U)
        {
            ret = RedInodeDataTruncate(pInode, 0ULL);
        }
    ...
    CINODE     *pInode)
{
  ...
  #if REDCONF_API_POSIX_LINK == 1
    else if(pInode->pBuffer->uNLink > 1U)
    {
        ret = RedInodeBranch(pInode);

        if(ret == 0)
        {
            pInode->pBuffer->uNLink--;
        }
    }
  #endif
    else
    ...
    CINODE     *pInode)
{
    ...
    else
    {
        ...
        RedBufferDiscard(pInode->pBuffer);
        pInode->pBuffer = NULL;
...
/** @brief Put all inode buffers except for the inode buffer.
...
#if REDCONF_READ_ONLY == 0
/** @brief Determine if an inode is writeable.

    @param ulInode          The inode number to examine.
    @param pfIsWriteable    On successful return, populated with whether the
                            inode is writeable.
    ...
    @retval -RED_EIO    A disk I/O error occurred.
*/
static REDSTATUS InodeIsWriteable(
    uint32_t    ulInode,
    bool       *pfIsWriteable)
{
    REDSTATUS   ret;

    if((pfIsWriteable == NULL) || !INODE_IS_VALID(ulInode))
    {
        REDERROR();
        ret = -RED_EINVAL;
    }
    ...
    CINODE *pInode)
{
    ...
    else if(!pInode->fWriteable)
    {
        ...
        if(ret == 0)
        {
            RedBufferBranch(pInode->pBuffer, InodeBlock(pInode->ulInode, bWhich));
            pInode->fWriteable = true;
            pInode->fDirty = true;
        }
    ...
    uint8_t    *pbWhich)
{
    ...
    else
    {
        ...
        /*  The writeable inode slot is the one which is free in the committed
            state, so look query the committed state metaroot.
        */
