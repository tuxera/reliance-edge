...
#endif
static uint8_t ImapNodeWhich(uint8_t bMR, uint32_t ulImapNode);
    ...
    bool       *pfAllocated)
{
    ...
    else
    {
        ...
        */
        if((bMR == (1U - gpRedCoreVol->bCurMR)) && !ImapNodeIsBranched(ulImapNode))
        {
            bMRToRead = 1U - bMR;
        }
        else
    ...
    IMAPNODE  **ppImap)
{
    ...
    else
    {
        ...
        */
        if(ImapNodeWhich(gpRedCoreVol->bCurMR, ulImapNode) == 0U)
        {
...
static bool ImapNodeIsBranched(
    uint32_t    ulImapNode)
{
    return ImapNodeWhich(0U, ulImapNode) != ImapNodeWhich(1U, ulImapNode);
}
...
uint32_t RedImapNodeBlock(
    ...
    uint32_t    ulImapNode)
{
    uint8_t     bWhichImap = ImapNodeWhich(bMR, ulImapNode);

    REDASSERT(ulImapNode < gpRedCoreVol->ulImapNodeCount);
    REDASSERT(bWhichImap <= 1U);

    return gpRedCoreVol->ulImapStartBN + (ulImapNode * 2U) + bWhichImap;
}


/** @brief Determine which copy of the imap node is current according to the
           given metaroot.

    An imap node has two locations on disk.  A bit in the metaroot bitmap
    indicates whether the first copy or the second copy is the valid one,
    according to that metaroot.

    @param bMR          Which metaroot to examine.
    @param ulImapNode   The imap node to examine.

    @return Either 0 or 1, indicating whether the first or second copy is valid.
*/
static uint8_t ImapNodeWhich(
    uint8_t     bMR,
    uint32_t    ulImapNode)
{
    uint8_t     bWhich;

    if((bMR > 1U) || (ulImapNode >= gpRedCoreVol->ulImapNodeCount))
    {
        REDERROR();
        bWhich = 0U;
    }
